...
+ (ZWTValidationResult) validatePassword:(NSString *)password isRequire:(BOOL)require
{
    @try
    {
        ...
        ZWTValidationResult result = [self validateLength:trimmedPassword min:6 max:50 isRequire:require];
...
+ (ZWTValidationResult) validateName:(NSString *)name isRequire:(BOOL)require
{
    ...
    ZWTValidationResult result = [self validateLength:trimmedName min:1 max:50 isRequire:require];
    
    if(result == ZWTValidationResultValid)
    {
        result = [self validateString:trimmedName againstRegExp:@"(^[A-Za-z .'-]{1,50})"];
